import { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X } from "lucide-react";
import whatsappGrupa from "@/assets/whatsapp-grupa.webp";

const STORAGE_KEY = "autazusa-whatsapp-modal-shown";

const WhatsAppModal = () => {
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    const hasSeen = localStorage.getItem(STORAGE_KEY);
    if (!hasSeen) {
      const timer = setTimeout(() => setIsOpen(true), 1200);
      return () => clearTimeout(timer);
    }
  }, []);

  const handleClose = () => {
    setIsOpen(false);
    localStorage.setItem(STORAGE_KEY, "true");
  };

  const handleJoin = () => {
    localStorage.setItem(STORAGE_KEY, "true");
    window.open("https://chat.whatsapp.com/KRYDgN90ak42HjPTX1pYMC", "_blank", "noopener,noreferrer");
  };

  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-white/60 backdrop-blur-md"
            onClick={handleClose}
          />
          <div className="fixed inset-0 z-[61] flex items-center justify-center p-4 pointer-events-none">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              transition={{ duration: 0.3, ease: "easeOut" }}
              className="relative pointer-events-auto w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl border border-black/5 bg-white"
            >
              <button
                onClick={handleClose}
                className="absolute right-3 top-3 z-10 p-2 rounded-full bg-white/90 text-usa-navy hover:bg-white transition-colors shadow-sm"
                aria-label="Zamknij"
              >
                <X className="w-5 h-5" />
              </button>

              {/* Header */}
              <div className="bg-usa-navy px-6 pt-6 pb-4 text-center">
                <span className="inline-block bg-usa-red text-white text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-3">
                  NOWOŚĆ
                </span>
                <h2 className="font-heading text-base md:text-lg font-black text-white uppercase leading-tight whitespace-nowrap">
                  OTWIERAMY NASZĄ GRUPĘ WHATSAPP!
                </h2>
              </div>

              {/* Image */}
              <div className="relative w-full bg-gray-100">
                <img
                  src={whatsappGrupa}
                  alt="Dołącz do grupy WhatsApp Auta z Ameryki USA"
                  className="w-full h-auto object-contain"
                />
              </div>

              {/* Body */}
              <div className="px-6 py-5 text-center">
                <p className="text-foreground/90 text-sm md:text-base leading-relaxed mb-6">
                  Szukasz auta z USA i chcesz jako jeden z pierwszych widzieć ciekawe okazje z aukcji? Stworzyliśmy grupę <strong>„Auta z Ameryki USA | Aukcje i okazje”</strong>.
                </p>
                <button
                  onClick={handleJoin}
                  className="w-full inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#128C7E] text-white font-bold text-lg uppercase tracking-wide px-8 py-4 rounded-lg transition-colors shadow-lg"
                >
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.746.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.134 1.585 5.929L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                  Dołącz do grupy
                </button>
                <button
                  onClick={handleClose}
                  className="mt-3 text-xs text-muted-foreground hover:text-foreground underline underline-offset-2 transition-colors"
                >
                  Nie teraz
                </button>
              </div>
            </motion.div>
          </div>
        </>
      )}
    </AnimatePresence>
  );
};

export default WhatsAppModal;
